...
func Eval(ID uint, voteid int, k []byte) {
	...
	// 更新记录的特定字段
	argument.VW0 = VW
	argument.Outputs0 = string(outputsJSON)
...
func Tallying(voteid int, N int, eta int) []*big.Int {
	...
	for w := 0; w < N; w++ {
		var outputStrings []string
		if err := json.Unmarshal([]byte(arguments[w].Outputs0), &outputStrings); err != nil {
			fmt.Println("Failed to parse Outputs0 JSON:", err)
			return nil
		}
...
func Verify(voteid int, N int, outs []*big.Int) []*big.Int {
	...
	for w := 0; w < N; w++ {
		...
		// 解析 Outputs0 的 JSON 字符串为字符串数组
		var outputStrings []string
		if err := json.Unmarshal([]byte(arguments[w].Outputs0), &outputStrings); err != nil {
			fmt.Println("Failed to parse Outputs0 JSON:", err)
			return nil
		}
		// fmt.Printf("Parsed Outputs0 JSON for argument %d: %v\n", w, outputStrings)

		// 转换为 big.Int 数组
		var outputs0 []*big.Int
		for _, outputStr := range outputStrings {
			...
			outputs0 = append(outputs0, output)
		}
		// fmt.Printf("Converted Outputs0 to big.Int array for argument %d: %v\n", w, outputs0)
